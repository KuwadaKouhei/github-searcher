...
- **取得の重複排除**: `generateMetadata`（§3.6）とページ本体が同じ `getRepository(owner, repo)` を呼ぶため、`React.cache` でラップしてリクエスト単位で1回の取得にまとめる（同一リクエスト内のメモ化であり、リクエストをまたぐ永続キャッシュではない）。
- **鮮度方針**: 永続キャッシュ（`revalidate` 等）は設けず、デフォルトの「毎回最新を取得」とする。理由は Star/Watcher 数など表示データが時々刻々変動するため、古い値を見せないことを優先する。Next.js 16 の既定（fetch を自動キャッシュしない方向）とも一致。
...
- `generateMetadata` でリポジトリ名・説明から動的に `title`（`{fullName} | Repo Finder`）と `description` を生成。取得失敗時は汎用タイトルにフォールバック（404/エラー表示はページ本体が担当）。
- 取得は §3.2 の `React.cache` 経由でページ本体と共有し、二重フェッチを避ける。
...
| ④ | E2E（Playwright・実装済み） | アプリ全体（実ブラウザ） | ハッピーパス1本（検索→結果→詳細→7項目→戻る）。GitHub API は page.route でモックし安定化 |

補足: テーマ切替は、トグルで `<html>` のテーマ属性が変わること・localStorageに保持されることを軽くテストに含める（任意）。

**E2E のブラウザ範囲**: chromium のみで検証する。理由は、本アプリがブラウザ依存の際どい実装（独自CSS hack・特定ブラウザ専用API等）を持たずモダンブラウザ前提（TECH_STACK.md）であり、E2E の目的が「主要動線が本物のブラウザで通る」最小保証であって互換性マトリクスの網羅ではないため。3ブラウザ化が必要になれば `playwright.config.ts` の `projects` に firefox / webkit を追加するだけで対応できる
